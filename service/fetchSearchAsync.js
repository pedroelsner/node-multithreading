...
const fetchSearchAsync = async (req, res, elastic) => {
    ...
    try {
        const response = await fetch("https://www.cvc.com.br/api/flights/Search/AsyncStream", {
            ...
            body: stringify(req.body.search),
            method: "post",
            timeout: 0,
            mode: "cors",
        });
        ...
        try {
            ...
            for await (const chunk of response.body) {
                ...
                try {
                    ...
                    let data = JSON.parse(chunkBlock);
                    console.log(`priceGroups: ${data.priceGroups.length} | completedWork: ${data.completedWork}`);

                    // Empty for next loop
                    chunkBlock = '';

                    if (data.priceGroups.length == 0) {
                        throw new Error('priceGroup empty')
                    }

                    // Add search id into all priceGroups
                    data.priceGroups = data.priceGroups.map(group => {
                        group.sid = res.locals.searchId;
                    ...
                    try {
                        ...
                        console.log('Error when save chunk elastic', err);
                    }
        ...
        console.error('Error when fetch search async');
        throw err;
    }
};

export default fetchSearchAsync;